...
namespace P1_ShubertFunction
{
    ...
    class Program
    {
        ...
        static public double shubertFuntion(double x1,double x2)
        {
            double a = 0, b = 0;
            for (int i = 1; i <= 5; i++)
            {
                a = a + (i * Math.Cos(((i + 1) * x1) + i));
                b = b + (i * Math.Cos(((i + 1) * x2) + i));
            }
            double ans = a * b;
            return ans;
        }
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n_chro; i++)
            {
                x1[i] = scaling(decoder(newPopulation[i]), -10, 10);
                x2[i] = scaling(decoder(newPopulation2[i]), -10, 10);
                y1[i] = shubertFuntion(x1[i], x2[i]);
            }
            y_fNew = fitness(y1);
            ...
            X1 = scaling(decoder(newPopulation[max_index]), -10, 10);
            X2 = scaling(decoder(newPopulation2[max_index]), -10, 10);
            Y1 = shubertFuntion(X1, X2);
            counter = 0;
