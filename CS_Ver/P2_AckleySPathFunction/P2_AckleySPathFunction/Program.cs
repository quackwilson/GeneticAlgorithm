...
namespace P2_AckleySPathFunction
{
    ...
    class Program
    {
        ...
        static double max_value, X1, X2, X3, X4, X5, Y1;
        static int max_index;
        static string X1s, X2s, X3s, X4s, X5s;
        static int counter;
        static double[] good_value = new double[10001];
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n_chro; i++)
            {
                x1[i] = scaling(decoder(newPopulation[i]), -32.768, 32.768);
                x2[i] = scaling(decoder(newPopulation2[i]), -32.768, 32.768);
                x3[i] = scaling(decoder(newPopulation3[i]), -32.768, 32.768);
                x4[i] = scaling(decoder(newPopulation4[i]), -32.768, 32.768);
                x5[i] = scaling(decoder(newPopulation5[i]), -32.768, 32.768);
                y1[i] = AFunction(x1[i], x2[i], x3[i], x4[i], x5[i]);
            }
            y_fNew = fitness(y1);
            ...
            X1 = scaling(decoder(newPopulation[max_index]), -32.768, 32.768);
            X2 = scaling(decoder(newPopulation2[max_index]), -32.768, 32.768);
            X3 = scaling(decoder(newPopulation3[max_index]), -32.768, 32.768);
            X4 = scaling(decoder(newPopulation4[max_index]), -32.768, 32.768);
            X5 = scaling(decoder(newPopulation5[max_index]), -32.768, 32.768);
            Y1 = AFunction(X1, X2, X3, X4, X5);
            counter = 0;
            good_value[counter] = Y1;
            Console.WriteLine(X1 + " " + X2 + " " + X3 + " " + X4 + " " + X5 + " " + Y1);
