...
namespace QZ2
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n_chro; i++)
            {
                x1[i] = scaling(decoder(newPopulation[i]), -512, 512);
                x2[i] = scaling(decoder(newPopulation2[i]), -512, 512);
            ...
            X1 = scaling(decoder(newPopulation[max_index]), -512, 512);
            X2 = scaling(decoder(newPopulation2[max_index]), -512, 512);
            Y1 = Function(X1, X2);
            counter = 0;
            ...
            for (int i = 0; i < 10001; i++)
            {
                ...
                using (var writer = new StreamWriter("./P5.csv", append))
