    ...
    class Program
    {
        ...
        static public double Function(double x1, double x2)
        {
            double a = 5 * x1 * x1,
                ...
                d = (5 * x1) / Math.PI,
                e = (x2 - c + d - 6) * (x2 - c + d - 6),
                f = 10 * (1 - (1 / (8 * Math.PI))) * Math.Cos(x1);
            double ans = e + f + 10;
            return ans;
        }
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n_chro; i++)
            {
                x1[i] = scaling(decoder(newPopulation[i]), -5, 10);
                x2[i] = scaling(decoder(newPopulation2[i]), 0, 15);
            ...
            X1 = scaling(decoder(newPopulation[max_index]), -5, 10);
            X2 = scaling(decoder(newPopulation2[max_index]), 0, 15);
            Y1 = Function(X1, X2);
            counter = 0;
